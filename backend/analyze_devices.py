#!/usr/bin/env python3
"""
Standalone device fingerprinting analysis script
"""
                ...
                for mac_addr in [src_mac, dst_mac, bss_mac]:
                    if mac_addr and mac_addr in mac_to_device:
                        ...
                        if (
                            ...
                            api_vendor = get_vendor_from_api(src_mac)
                            if api_vendor:
                                stats["vendor"] = api_vendor
                                print(
                                    f"[*] Found vendor for {device_name} ({src_mac}): {api_vendor}"
                                )
                            else:
...
def main():
    ...
    if not capture_files:
        print("[ERROR] No capture files found in downloads directory")
        return

    # Get the latest file (assuming capture-01.cap for now)
    latest_file = os.path.join(downloads_dir, "capture-01.cap")
    ...
    print(f"[*] Router BSSID: {BSSID}")
    print(f"[*] Known devices: {len(DEVICE_MACS)}")
    print("=" * 60)
    ...
    try:
        ...
        print(f"[*] Found {len(devices)} active devices from known MAC addresses")
        print("=" * 60)

        if devices:
            for i, device in enumerate(devices, 1):
                print(f"\n[DEVICE {i}] {device['device_name'].upper()}")
                print(f"  MAC Address: {device['mac_address']}")
                print(f"  Vendor: {device['vendor']}")
                print(f"  Total Packets: {device['total_packets']}")
                print(f"  Confidence: {device['confidence']:.1%}")
                print(
                    f"  Connected to Router: {'Yes' if device['connected_to_router'] else 'No'}"
                )

                print(f"  Packet Types:")
                print(
                    f"    Data: {device['packet_types']['data']['count']} ({device['packet_types']['data']['percentage']}%)"
                )
                print(
                    f"    Management: {device['packet_types']['management']['count']} ({device['packet_types']['management']['percentage']}%)"
                )
                print(
                    f"    Control: {device['packet_types']['control']['count']} ({device['packet_types']['control']['percentage']}%)"
                )

                print(f"  First Seen: {device['first_seen']}")
                print(f"  Last Seen: {device['last_seen']}")

                if device["avg_signal_strength"]:
                    print(f"  Avg Signal Strength: {device['avg_signal_strength']} dBm")

                print("-" * 40)
        else:
            print("\n[*] No known devices found in the capture file.")
            print("[*] This could mean:")
            print("    - The devices were not active during capture")
            print("    - The MAC addresses don't match")
            print("    - The capture file doesn't contain 802.11 frames")

        # Save results to JSON file
        output_file = os.path.join(downloads_dir, "device_analysis_results.json")
